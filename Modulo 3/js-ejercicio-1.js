...
const fromCSV = (csv, nAttrs = 0) => {
  ...
  const index =
    nAttrs === 0 || nAttrs > headerFields.length ? headerFields.length : nAttrs;

  const result = data.map((row) => {
    const values = row.split(",");
    return headerFields.slice(0, index).reduce((person, field, j) => {
      person[field] = values[j];
      return person;
    }, {});
  });
